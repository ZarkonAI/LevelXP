START = (
    "Привет. Это <b>FitXP</b> — XP за реальные подходы.\n\n"
    "Ты записываешь тренировку в чате — персонаж получает опыт и прокачку.\n"
    "Начни с раздела <b>🏋️ Тренировка</b>."
)

MENU = "Выбери раздел:"
ACHIEVEMENTS_TITLE = "<b>🏆 Достижения</b>"

TRAINING_CHOOSE = "Как записываем тренировку?"
CHOOSE_MODE = "Выбери режим:"
CHOOSE_CATEGORY = "Выбери мышечную группу или действие:"
CHOOSE_EXERCISE = "Выбери упражнение"
SEARCH_PROMPT = "Введи запрос"
SEARCH_EMPTY = "Ничего не найдено."
SEARCH_RESULTS_TITLE = "Результаты (стр. {page}/{total}). Выбери упражнение:"
USE_INLINE_BACK_HINT = "Используй кнопки ⬅️ Назад под сообщением."
ENTER_CUSTOM_NAME = "Напиши название упражнения одним сообщением."
CHOOSE_PRIMARY_MUSCLE = "Выбери основную мышечную группу:"
WEIGHT_HINT_BODYWEIGHT = "Вес — это вес снаряда/доп. отягощения. Для подтягиваний без веса введи 0."
ENTER_WEIGHT = "Введи вес (кг). Если без веса — 0.\n" + WEIGHT_HINT_BODYWEIGHT
ENTER_WEIGHT_EXTERNAL = "Вес снаряда/отягощения (кг). Если без веса — 0."
ENTER_WEIGHT_BODYWEIGHT_PLUS = "Доп. вес (0 если нет). Эффективный вес = вес тела + доп. вес."
ENTER_WEIGHT_ASSIST = "Помощь гравитрона (кг). Эффективный вес = вес тела - помощь."
WEIGHT_MODE_BODY_WEIGHT_WARNING = "⚠️ Вес тела не указан: эффективный вес и XP могут считаться неточно."
ENTER_REPS = "Введи повторения (например 8)."
ENTER_SETS = "Введи количество подходов (например 3)."
ENTER_REST_SINGLE = "Введи отдых между подходами (в минутах). Можно дробно: 2,5"
ENTER_REST_PATTERN = (
    "Введи отдых после КАЖДОГО подхода (между подходами) через запятую.\n"
    "Пример для 3 подходов: 2, 2.5\n"
    "(нужно 2 значения — после 1 и 2 подхода)"
)
CONFIRM = "Проверь запись и нажми ✅ Сохранить."
SAVED = "Записал. Тренировка учтена."
ERR_NUMBER = "Не понял значение. Введи число. Можно дробно: 2,5"
ERR_RANGE = "Проверь ввод. Значение выглядит неправдоподобно. Попробуй ещё раз."
ERR_EXERCISE_NAME = "Название упражнения не подходит. Используй 2–60 символов: буквы/цифры/пробел/дефис, без ссылок и оскорблений."

HISTORY_TITLE = "<b>История тренировок</b> (последние 10)\nВыбери пункт из списка:"
HISTORY_EMPTY = "История пустая. Запиши первую тренировку через 🏋️ Тренировка."
HISTORY_CARD_TITLE = "Детали тренировки"
STATUS_DONE = "✅ выполнено"
STATUS_PLANNED = "🕓 план"
REPEAT_DONE = "Создал новую тренировку на сегодня на основе выбранной."
EDIT_START = "Исправление записи. Введи новый вес (кг)."
EDIT_ENTER_REPS = "Введи новое количество повторов."
EDIT_ENTER_SETS = "Введи новое количество подходов."
EDIT_ENTER_REST_SINGLE = "Введи новый отдых между подходами (в минутах). Можно дробно: 2,5"
EDIT_ENTER_REST_PATTERN = "Введи новый отдых по подходам через запятую (в минутах)."
EDIT_CONFIRM = "Проверь правки и нажми ✅ Сохранить правки."
EDIT_SAVED = "Правки сохранены."

TEMPLATES_TITLE = "<b>Шаблоны</b>\nВыбери шаблон:"
TEMPLATES_EMPTY = "Шаблонов нет. Сохрани шаблон из истории."
ASK_TEMPLATE_NAME = "Как назовём шаблон? (например: Грудь/спина)"
TEMPLATE_SAVED = "Шаблон сохранён."
TEMPLATE_REPEAT_CONFIRM = "Применить шаблон сегодня?"
TEMPLATE_REPEATED = "Шаблон применён. Тренировка создана на сегодня."
HISTORY_TEMPLATE_MODE = "Куда сохранить тренировку из истории?"
TEMPLATE_PICK_EXISTING = "Выбери шаблон, в который добавить тренировку:"
TEMPLATE_APPENDED = "Добавил в шаблон."
TECHNIQUE_LINK_PREFIX = "🖼 Техника:"

TECH_ERROR = "Похоже, произошла техническая ошибка. Попробуй ещё раз или нажми ↩️ В меню."
CANCELLED = "Ок, отменил. Возвращаюсь в меню."
ACHIEVEMENT_UNLOCKED = "🏆 Достижение открыто: {name}"
PING = "ok"

VERSION = "FitXP v1.0"

ACHIEVEMENTS_EMPTY = "Пока достижений нет. Запиши тренировку, чтобы открыть первое."

SETTINGS_TEXT = "<b>⚙️ Настройки</b>\nЕдиницы: <b>{units}</b>"
SETTINGS_UNITS_PROMPT = "Выбери единицы измерения:"
SETTINGS_EXERCISE_LANG_PROMPT = "Выбери язык упражнений:"
SETTINGS_TRANSLATE_MODE_PROMPT = "Режим ручного перевода названий упражнений:"
ONBOARDING_WEIGHT_PROMPT = "Укажи вес тела в кг (например: 80 или 80,5)."
ONBOARDING_WEIGHT_WARNING = (
    "⚠️ Вес тела не указан: без него часть упражнений будет считаться неточно "
    "(подтягивания/гравитрон)."
)
ONBOARDING_HEIGHT_PROMPT = "Укажи рост в см (опционально, например: 175)."
ONBOARDING_DONE = "Профиль сохранён ✅"
SETTINGS_WEIGHT_TITLE = "<b>⚖️ Вес тела</b>\nТекущее значение: <b>{value}</b>"
SETTINGS_WEIGHT_EMPTY = "не указан"
SETTINGS_WEIGHT_UPDATED = "Вес тела обновлён ✅"
SETTINGS_HEIGHT_PROMPT = "Введи рост в см (целое число 100..230) или выбери «⏭️ Пропустить»."
SETTINGS_HEIGHT_UPDATED = "Рост обновлён ✅"
SETTINGS_HEIGHT_SKIPPED = "Рост очищен ✅"
SETTINGS_HEIGHT_INVALID = "Введи целое число от 100 до 230."
SETTINGS_FREQ_PROMPT = "Выбери частоту тренировок в неделю (1..7):"
SETTINGS_FREQ_UPDATED = "Частота тренировок обновлена ✅"
SETTINGS_FREQ_INVALID = "Выбери значение от 1 до 7."
SETTINGS_DURATION_PROMPT = "Выбери среднюю длительность тренировки:"
SETTINGS_DURATION_UPDATED = "Длительность тренировки обновлена ✅"
SETTINGS_DURATION_INVALID = "Выбери один из вариантов: 25, 45, 60, 90, 120."
TECHNIQUE_CAPTION = "{display_name}\n🖼 техника"
CHOOSE_EXERCISE_FROM_LIST = "Выбери пункт из списка"
RU_NAME_PROMPT = "Введи русское название для упражнения или '-' чтобы пропустить."

UNAVAILABLE = "Недоступно"
TRANSLATE_IMAGE_MISSING = "⚠️ Не удалось показать изображение. Если есть ссылка на технику — проверь image_url в БД."

TRANSLATE_CARD_TEMPLATE = "<b>EN:</b> {name_en}\n<b>Мышцы:</b> {muscles}\n<b>Оборудование:</b> {equipment}"
TRANSLATE_SKIPPED = "Пропустил."
TRANSLATE_SAVED = "✅ Сохранил: {name_en} → {name_ru}"
TRANSLATE_KEEP_EN = "✅ Оставил EN: {name_en}"
TRANSLATE_ALL_DONE = "Все переведено в этой категории ✅"
TRANSLATE_CHOOSE_ACTION = "Выбери действие для перевода:"

EXERCISE_PREVIEW_TEMPLATE = (
    "<b>{display_name}</b>\n"
    "<b>Мышцы:</b> {muscles}\n"
    "<b>Оборудование:</b> {equipment}\n"
    "<b>Техника:</b>\n{technique}"
)

EXERCISE_CARD_HINT = "Выбери действие: добавь в избранное или продолжи запись."
FAVORITE_ADDED = "⭐ Добавил в избранное"
FAVORITE_REMOVED = "⭐ Убрал из избранного"
FEATURED_ENABLED = "🔥 Рекомендуемое включено"
FEATURED_DISABLED = "🔥 Рекомендуемое выключено"


HELP_TEXT = (
    "<b>❓ Помощь</b>\n\n"
    "FitXP — бот для записи тренировок и прокачки персонажа через XP.\n\n"
    "• Как записывать: открой <b>🏋️ Тренировка</b> и следуй шагам.\n"
    "• Как учитывать вес: укажи вес снаряда/доп. отягощения, если без веса — введи 0.\n"
    "• Как предложить упражнение: нажми кнопку ниже <b>➕ Предложить упражнение</b>."
)
SUPPORT_WRITE_PROMPT = "Напиши сообщение в поддержку одним текстом."
SUPPORT_EXERCISE_PROMPT = "Название (как можешь), мышцы, оборудование, ссылка/описание"
SUPPORT_EMPTY_TEXT = "Сообщение пустое. Отправь текст одним сообщением."
SUPPORT_TEXT_ONLY = "Нужен текст одним сообщением."
SUPPORT_SENT = "✅ Сообщение отправлено в поддержку. Мы ответим здесь."
SUPPORT_EXERCISE_SENT = "✅ Заявка на упражнение отправлена. Мы рассмотрим и добавим при необходимости."
